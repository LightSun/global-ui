...
inline void write_data_to_file(const std::string &fn,
  ...
  if (f == nullptr) {
      fprintf(stderr, "Cannot open file [%s] for writing. (Does the directory exist?)\n", fn.data());
      //TI_ERROR("Cannot open file [{}] for writing. (Does the directory exist?)",fn);
      TI_ASSERT(f != nullptr);
  }
  if (taichi::ends_with(fn, ".tcb.zip")) {
    ...
    std::fclose(f);
      ...
      fprintf(stderr, "File must end with .tcb or .tcb.zip. [Filename = %s]\n", fn.data());
 ...
 private:
  ...
  void process(const TArray<T, n> &val) {
    if (writing) {
      ...
      std::vector<
          std::conditional_t<std::is_same<Traw, bool>::value, taichi::uint8, Traw>>
          tmp(n);
      for (std::size_t i = 0; i < n; i++) {
        this->process(tmp[i]);
      }
